    ...
    if (item.finalidade === 'venda') {
      return (
        <div className="flex flex-col">
          ...
          <span className="text-[8px] font-black text-indigo-500 uppercase tracking-widest">Preço de Venda</span>
        </div>
      );
    }
    if (item.finalidade === 'locacao') {
      return (
        <div className="flex flex-col">
          <span className="text-xs font-black text-slate-900 uppercase tracking-tight">
            {formatCurrency(item.valor_locacao)}
          </span>
          <span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest">Aluguel / Mensal</span>
        </div>
      );
    }
    ...
    return (
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-black text-slate-900">{formatCurrency(item.valor_venda)}</span>
          <span className="text-[7px] font-bold text-slate-400 uppercase">VENDA</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-black text-slate-900">{formatCurrency(item.valor_locacao)}</span>
          <span className="text-[7px] font-bold text-slate-400 uppercase">LOCAÇÃO</span>
        </div>
      </div>
    );
  };
        ...
        <main className="p-8 md:p-12 max-w-[1600px] mx-auto w-full space-y-12 animate-in fade-in duration-700">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => navigate('/imoveis/novo')} className="px-8 py-4 bg-slate-950 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-slate-200 hover:bg-indigo-600 transition-all active:scale-95 flex items-center gap-3">
                        ...
                        <td className="px-10 py-6">
                          <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-2">
                              ...
                              {item.destaque && <span className="bg-amber-50 text-amber-600 border border-amber-100 px-2 py-0.5 rounded text-[8px] font-black uppercase">⭐ Destaque</span>}
                            </div>
                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{item.bairro}, {item.cidade}</span>
                          </div>
                        </td>
                        <td className="px-10 py-6">
                          {renderValorPrincipal(item)}
                        </td>
                        <td className="px-10 py-6">
